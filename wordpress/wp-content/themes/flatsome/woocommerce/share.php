...
if ( get_theme_mod( 'social_icons_tooltip', 1 ) ) {
	$classes .= ' tooltip';
}
?>
...
<div class="yith-wcwl-share social-icons share-icons share-row relative">
	...
	<?php if ( $share_facebook_enabled ) : ?>
		<a target="_blank" rel="noopener nofollow" class="facebook <?php echo $classes; ?>" href="https://www.facebook.com/sharer.php?u=<?php echo urlencode( $share_link_url ); ?>&p[title]=<?php echo esc_attr( $share_link_title ); ?>" title="<?php esc_html_e( 'Facebook', 'yith-woocommerce-wishlist' ) ?>" aria-label="<?php esc_html_e( 'Share on Facebook', 'flatsome' ); ?>">
			<?php echo get_flatsome_icon( 'icon-facebook' ); ?>
		</a>
	<?php endif; ?>

	<?php if ( $share_twitter_enabled ) : ?>
		<a target="_blank" rel="noopener nofollow" class="twitter <?php echo $classes; ?>" href="https://twitter.com/share?url=<?php echo urlencode( $share_link_url ); ?>&amp;text=<?php echo esc_attr( $share_twitter_summary ); ?>" title="<?php esc_html_e( 'Twitter', 'yith-woocommerce-wishlist' ); ?>" aria-label="<?php esc_html_e( 'Share on Twitter', 'flatsome' ); ?>">
			<?php echo get_flatsome_icon( 'icon-twitter' ); ?>
		</a>
	<?php endif; ?>

	<?php if ( $share_pinterest_enabled ) : ?>
		<a target="_blank" rel="noopener nofollow" class="pinterest <?php echo $classes; ?>" href="https://pinterest.com/pin/create/button/?url=<?php echo urlencode( $share_link_url ); ?>&amp;description=<?php echo esc_attr( $share_summary ); ?>&amp;media=<?php echo esc_attr( $share_image_url ); ?>" title="<?php esc_html_e( 'Pinterest', 'yith-woocommerce-wishlist' ); ?>" onclick="window.open(this.href); return false;" aria-label="<?php esc_html_e( 'Pin on Pinterest', 'flatsome' ); ?>">
			<?php echo get_flatsome_icon( 'icon-pinterest' ); ?>
		</a>
	<?php endif; ?>

	<?php if ( $share_email_enabled ) : ?>
		<a rel="nofollow" class="email <?php echo $classes; ?>" href="mailto:?subject=<?php echo esc_attr( apply_filters( 'yith_wcwl_email_share_subject', $share_link_title ) ); ?>&amp;body=<?php echo esc_attr( apply_filters( 'yith_wcwl_email_share_body', urlencode( $share_link_url ) ) ); ?>&amp;title=<?php echo esc_attr( $share_link_title ); ?>" title="<?php esc_html_e( 'Email', 'yith-woocommerce-wishlist' ); ?>" aria-label="<?php esc_html_e( 'Email to a Friend', 'flatsome' ); ?>">
			<?php echo get_flatsome_icon( 'icon-envelop' ); ?>
		</a>
	<?php endif; ?>

	<?php if ( $share_whatsapp_enabled ) : ?>
		<a target="_blank" rel="noopener" class="whatsapp show-for-medium <?php echo $classes; ?>" href="<?php echo esc_attr( $share_whatsapp_url ); ?>" data-action="share/whatsapp/share" title="<?php esc_html_e( 'WhatsApp', 'yith-woocommerce-wishlist' ); ?>" aria-label="<?php esc_html_e( 'Share on WhatsApp', 'flatsome' ); ?>">
			<?php echo get_flatsome_icon( 'icon-whatsapp' ); ?>
		</a>
	<?php endif; ?>
